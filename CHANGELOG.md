...
- **refactor:** removed the `Common UI` sample entry from `package.json` (its code is now shipped as runtime). A thin demo scene should be re-added under `Samples~/` before publishing.
